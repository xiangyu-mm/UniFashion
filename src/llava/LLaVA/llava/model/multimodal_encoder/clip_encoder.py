...
class C_B_LIPVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        super().__init__()

        self.is_loaded = False
        blip_model_name = "blip2_cir_cls"
        ...
        self.blip_model.to(dtype=torch.float16)
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            blip_features = self.blip_model.extract_target_features(images.to(device=self.device, dtype=self.dtype))
            print(blip_features)

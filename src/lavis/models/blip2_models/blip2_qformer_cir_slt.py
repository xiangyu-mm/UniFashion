...
class Blip2QformerCirSlt(Blip2Base):
    ...
    def __init__(
        ...
        ###============== RQ-VAE ===================###
        # self.encoder = Encoder(in_features=768, out_features=256)
        # self.decoder = Decoder(in_features=256, out_features=768)

        self.quantizer = RQBottleneck(latent_shape=[768],
                                      code_shape=[8],
                                      n_embed=16384,
                                      decay=0.99,
    ...
    def forward(self, samples):
        ...
        fusion_query = fusion_output.last_hidden_state[:, 32, :]

        fusion_vft = F.normalize(self.vision_proj(fusion_output.last_hidden_state[:, : query_tokens.size(1), :]), dim=-1)

        z_q, quant_loss, code = self.quantizer(fusion_query)
        z_e = self.text_proj(z_q)

        edited_image_feats = F.normalize(z_e, dim=-1)
        ...
        sim_q2q = torch.matmul(
            edited_image_feats.unsqueeze(1).unsqueeze(1), target_feats.permute(0, 2, 1)
        ).squeeze()
        ...
        sim_q2q, _ = sim_q2q.max(-1)
        sim_i2t, _ = sim_t2q.max(-1)

        sim_q2q = sim_q2q / self.temp
        ...
        loss_iti = F.cross_entropy(sim_q2q, targets)
        ...
        loss_align = F.mse_loss(fusion_vft,
                                target_feats)

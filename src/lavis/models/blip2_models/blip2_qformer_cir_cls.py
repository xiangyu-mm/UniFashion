...
class Blip2QformerCirCls(Blip2Base):
    ...
    def forward(self, samples):
        ...
        # attention_mask_code = torch.cat([query_atts, code_tokens.attention_mask], dim=1)
        ...
        sim_t2q = torch.matmul(
            text_only_feat.unsqueeze(1).unsqueeze(1), image_feats.permute(0, 2, 1)
        ).squeeze()

        sim_q2t = torch.matmul(
            image_feats.unsqueeze(1), text_only_feat.unsqueeze(-1)
        ).squeeze()
        ...
        for b in range(bs):
            ...
            target_embeds_neg.append(image_embeds[neg_idx])
    ...
    @torch.no_grad()
    def extract_features(self, samples, mode="text"):
        """

...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        # c is clip image embedding, x is encoded by acuto encoder
        ...
        cond_dict = dict(c_crossattn=batch['txt'], c_concat=control, txt=batch['txt'])
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        log = dict()
        annotation_file = open('/home/data2/xiangyu/Code/StableVITON/logs/captions.json', "w")
        ...
        captions = c['txt'][:N]
        annotation_file.write(json.dumps({"captions": captions, "images": batch['cloth_fn']}))

        c_cat = [i[:N] for i in c["c_concat"]]
        c = c["c_crossattn"][:N]

        # if c.ndim == 4:
        c = self.get_learned_text_conditioning(c)
        N = min(z.shape[0], N)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx):
        ...
        bs = z.shape[0]
        c_crossattn = c["c_crossattn"][:bs]
        # if c_crossattn.ndim == 4:
        c_crossattn = self.get_learned_text_conditioning(c_crossattn)
        ...
        for x_sample, cloth, gt, fn, recon, caption in zip(x_samples, batch["cloth"], batch["image"], batch["img_fn"], x_recon, batch["txt"]):
            ...
            annotation_file = open(f'/home/data2/xiangyu/Code/StableVITON/logs/captions/{fn}.json', "w")
            # captions = batch['txt']
            annotation_file.write(json.dumps({"captions": caption, "images": fn}))

...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.instantiate_cond_text_stage(cond_stage_text_config)
    ...
    # define the T5 encoder for text embedding
    def instantiate_cond_text_stage(self, config):
        text_model = instantiate_from_config(config)
        self.cond_text_model = text_model.eval()
        self.cond_text_model.train = disabled_train
        for param in self.cond_text_model.parameters():
            param.requires_grad = False
    ...
    def get_learned_text_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_text_model, 'encode') and callable(self.cond_text_model.encode):
                c = self.cond_text_model.encode(c)
            else:
                c = self.cond_stage_model(c)
        else:
            assert hasattr(self.cond_stage_model, self.cond_stage_forward)
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        # pbe negative condition
            ...
            c["c_crossattn"] = [self.get_learned_text_conditioning(c["txt"])]
            c["txt"] = [self.get_learned_text_conditioning(c["txt"])]
            if self.u_cond_prop < self.u_cond_percent:
                c["c_crossattn"] = [self.learnable_vector.repeat(x.shape[0],77,1)]
            return self.p_losses(x, c, t, *args, **kwargs)
